...
class PromoCode(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    code: Mapped[str] = mapped_column(String(64), nullable=False, unique=True, index=True, comment="FIT-8XK29-QJ4M7")
    ...
    employee_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    redeemed_by_id: Mapped[UUID | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
